...
interface InternalApiOptions extends ApiOptions { _retried?: boolean }
...
interface AuthResponse { token: string; user: AuthUser; message: string }
...
export async function authRefresh() {
  const data = await api<AuthResponse>(`/auth/refresh`, { method: 'POST', auth: true });
  setAuthToken(data.token);
  return data;
}
...
export async function authVerify() {
  ...
  return api<{ valid: boolean; user: AuthUser }>(`/auth/verify`, { method: 'POST', auth: true });
}
