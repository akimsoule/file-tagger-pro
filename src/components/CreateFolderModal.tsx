...
export function CreateFolderModal({ open, parentId, onClose, onCreated }: CreateFolderModalProps) {
  ...
  const { createFolder } = useFileContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (disabled) return;
    try {
  const payload: { name: string; description?: string; color?: string; parentId?: string } = { name: name.trim(), description: description.trim() || undefined, color };
  if (parentId && parentId !== 'root') payload.parentId = parentId;
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder="Nom du dossier" />
          </div>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={disabled}>{createMut.isPending ? 'Création...' : 'Créer'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
