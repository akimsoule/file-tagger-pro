...
async function buildTree(userId: string): Promise<TreeFolderDTO | null> {
  const [folders, documents] = await Promise.all([
    prisma.folder.findMany({ where: { ownerId: userId }, orderBy: { createdAt: 'asc' } }),
    prisma.document.findMany({ where: { ownerId: userId }, orderBy: { createdAt: 'asc' } })
  ]);
  ...
  function buildFolder(f: any): TreeFolderDTO { // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    return {
      ...
      documents: (docsByFolder.get(f.id) || []).map((d: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
        id: d.id,
        name: d.name,
        type: d.type,
        size: d.size,
  ...
  if (rootFolder) {
    ...
    rootTree = {
      ...
      documents: (docsByFolder.get(null) || []).map((d: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
        id: d.id,
        name: d.name,
        type: d.type,
        size: d.size,
