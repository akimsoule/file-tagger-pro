...
function buildBasicFilters(
  userId: string,
  params: { type?: string | null; category?: string | null; tag?: string | null },
): BasicFilters {
...
function buildAdvancedFilters(
  userId: string,
  body: AdvancedSearchInput,
): { filters: BasicFilters; query: string; limit: number } {
  const filters: BasicFilters = { ownerId: userId };
  ...
  if ((body as any).minSize || (body as any).maxSize) {
    // conserver compat pour noms existants
    filters.fileSize = {};
...
async function handleAdvancedSearch(request: Request, user: AuthUser) {
  ...
  try {
    const { filters, query, limit } = buildAdvancedFilters(
      user.userId,
      parseResult.data as AdvancedSearchInput,
    );
    const searchFilters = { query, ...filters, limit };
    const results = await searchService.searchDocuments(searchFilters, user.userId);
    return createSuccessResponse({
