...
export default handleErrors(async (request: Request, _context: Context) => {
  ...
  if (request.method === "POST") {
    if (action === "upsert") {
      ...
      const { documentId, embedding, model } = parsed.data as {
        documentId: string;
        embedding: number[];
        model?: string;
      };
      if (!documentId || !Array.isArray(embedding))
        return createErrorResponse("documentId et embedding requis", 400);
      const res = await embeddingService.upsertEmbedding({
        documentId,
        embedding,
        model: model || "local-embedding",
      });
    ...
    if (action === "similar-vector") {
      const parsed = await safeJsonParse(request);
      if (!parsed.success) return createErrorResponse(parsed.error!, 400);
      const { vector, limit } = parsed.data as {
        vector: number[];
        limit?: number;
      };
