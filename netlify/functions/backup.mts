...
async function handleCreateBackup(request: Request, user: AuthUser) {
  ...
  if (!["full", "incremental", "documents-only"].includes(type)) {
    return createErrorResponse(
      "Type de sauvegarde invalide. Valeurs autorisées: full, incremental, documents-only",
      400,
    );
  }
  ...
  try {
    ...
    return createSuccessResponse(
      ...
      202,
    ); // Accepted - processus asynchrone
  } catch (error) {
    console.error("Erreur lors de la création de la sauvegarde:", error);
    const message =
      error instanceof Error ? error.message : "Erreur lors de la création de la sauvegarde";
