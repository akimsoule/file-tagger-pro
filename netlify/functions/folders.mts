      ...
      switch (request.method) {
        case "GET":
          if (url.pathname.endsWith("/folders")) {
            return user
              ? await handleGetFolders(request, user)
              : createErrorResponse("Utilisateur requis", 401);
          } else if (url.pathname.includes("/path")) {
            return user
              ? await handleGetFolderPath(request, user)
              : createErrorResponse("Utilisateur requis", 401);
          } else {
            return user
              ? await handleGetFolder(request, user)
              : createErrorResponse("Utilisateur requis", 401);
          }
        case "POST":
          if (url.pathname.endsWith("/folders")) {
            return user
              ? await handleCreateFolder(request, user)
              : createErrorResponse("Utilisateur requis", 401);
          } else if (url.pathname.includes("/move-document")) {
            return user
              ? await handleMoveDocument(request, user)
              : createErrorResponse("Utilisateur requis", 401);
          }
          break;
        case "PUT":
          return user
            ? await handleUpdateFolder(request, user)
            : createErrorResponse("Utilisateur requis", 401);
        case "DELETE":
          return user
            ? await handleDeleteFolder(request, user)
            : createErrorResponse("Utilisateur requis", 401);
      }
    }
